// =============================================================
// WUPI CDP PLAYTEST — 50-turn Fable simulation driver.
// Runs 50 player turns via fable_send, capturing per-turn:
//   • event types + scene_events (bracket commands)
//   • final narrator text + hygiene (markers, hyphens, fences)
//   • failures/errors/timeouts
//   • elapsed time
// After each turn, snapshots the live schema for subsystem analysis.
// Writes a full log to logs/cdp_playtest_50_turn_<ts>.log.
// =============================================================
...
// The 50 turns. Designed to exercise EVERY subsystem:
//  - social/dialogue (NPCs, relationships, persuade/deceive)
//  - stealth/skill checks (sneak, lockpick, disguise)
//  - combat (triggers referees + injury/heatsink)
//  - inventory (acquire items, equip, consume, store)
//  - travel (move between nodes)
//  - time/weather (advance hours, day/night)
//  - world autonomy (leave NPCs to act off-screen)
const TURNS = [
  // ── ACT 1: The Crooked Lantern (social + setup) ──
  ...
  // ── ACT 2: Market Square → Warehouse (travel + stealth) ──
  ...
  // ── ACT 3: The Warehouse (inventory acquisition + danger) ──
  ...
  // ── ACT 4: The Confrontation (social OR combat branch) ──
  ...
  // ── ACT 5: Return to Mara (delivery + consequence) ──
  ...
  // ── ACT 6: Equipment + preparation (inventory focus) ──
  ...
  // ── ACT 7: Disguise in action (disguise gate + skill) ──
  ...
  // ── ACT 8: Combat + injury (heatsink test) ──
  ...
  // ── ACT 10: World autonomy + resolution ──
  ...
  `I look around at my new surroundings — the rolling grey hills, the distant line of a forest, a thread of smoke on the horizon that might be a farmstead or a camp. I take stock of myself: my wounds, my gear, my coin, my options. The courier's life is behind me now. This is the first day of something new. I shouldered my pack and set off toward the distant smoke.`,
];
...
async function runTurn(i, text, log) {
  ...
  const full = await evalPage(`(() => {
    ...
    return {
      nEvents: events.length, types: events.map(e => e.type),
      streamed: chunks,
  ...
  // Console summary (one line per turn for the live monitor).
  ...
  if (scene.length) log.push(`        scene: ${scene.join(', ')}`);
  if (snap) {
    const inj = snap.player.injured.length ? ` injured:[${snap.player.injured.join(',')}]` : '';
    log.push(`        loc:${snap.loc} mode:${snap.mode} w:${snap.weather} | equip:[${snap.player.equip.join(',')}] belt:[${snap.player.belt.join(',')}] pack:[${snap.player.pack.join(',')}]${inj} | npc:${snap.presences.join(',')} rumor:${snap.rumors} rel:${snap.relationships}`);
  }
...
async function main() {
  ...
  const logPath = `C:/WUPI/logs/cdp_playtest_50_turn_${ts}.log`;
  const jsonPath = `C:/WUPI/logs/cdp_playtest_50_turn_${ts}.json`;
  const log = [];
  const results = [];
  log.push(`=== WUPI 50-TURN CDP PLAYTEST — ${new Date().toISOString()} ===`);
  log.push(`Card: cinderfen | Player: vera | turns: ${TURNS.length}`);
  console.log(log[0]);

  // --- Establish the Fable session before any turns. ---
  // fable_end any prior session first (idempotent if none), then fable_start
  // a fresh run on the cinderfen card. player_id is omitted (optional) since
  // no SavedPlayer exists; the card's default player_name (Vera) is used.
  try {
  ...
  for (let i = 0; i < TURNS.length; i++) {
    ...
    try {
      ...
      // Print the final narrator text (truncated for console).
      const txt = (r.text || '').slice(0, 600);
      console.log(`\n${txt}${r.text && r.text.length > 600 ? '\n  …[truncated]' : ''}`);
    ...
    // Flush the log after each turn so partial progress survives a crash.
    fs.writeFileSync(logPath, log.join('\n'));
