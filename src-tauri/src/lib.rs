...
pub struct AppState {
    ...
    /// (2026-08-16 bug 8) Fable session GENERATION counter — bumped by every
    /// session-boundary path (`fable_start` / `fable_load_save` / `fable_end`)
    /// as soon as its in-flight guard passes. `active_card_id` alone cannot
    /// detect a SAME-card load (the world-progression tick's identity guard
    /// blind spot): the tick's LLM pass runs ~1-3s AFTER the arming turn's
    /// cancel slot cleared, so a Load issued right after a beat lands passes
    /// every in-flight guard while the tick is still mid-flight — without
    /// this counter its post-LLM re-check passes and the dead timeline's
    /// delta lands on the freshly loaded world (re-populating just-drained
    ...
    /// `active_fable_cancel` (last-writer-owns), overwriting the LIVE narrator
    /// turn's token — a `fable_stop` during the overlap then signaled only the
    /// retrack decode and the narrator stream ran to completion. Distinct slot
    /// per consumer (the same Bug #7 §2C rule the slice/creator/SD slots
    /// follow); `fable_stop` signals BOTH fable-side slots.
    pub active_retrack_cancel: Arc<std::sync::Mutex<Option<llm::CancelToken>>>,
    ...
    /// Cached API HTTP backend (P3 #16): fable_send / fable_regenerate_slice /
    /// creator_assistant_turn used to construct a fresh `HttpBackend` (new
    /// reqwest Client — connection pool + TLS handshake) EVERY turn. The cache
...
impl AppState {
    fn new() -> Self {
        Self {
            ...
            http_backend_cache: Arc::new(std::sync::Mutex::new(None)),
...
/// sibling). SD checkpoints are `.safetensors` (canonical) or `.gguf`;
/// `pick_sd_checkpoint` prefers an assembled multi-file GGUF layout, then
/// the largest file. Returns `None` when absent — the SD
...
///    BOTH a `.gguf` and a CLIP-G sibling (`clip_g*` / `text_encoder_2*`
///    stems — the same needles `scene_art::resolve_multi_file_layout`
///    matches), the largest `.gguf` wins. This is the path sd.cpp currently
///    loads cleanly (pre-quantized UNet, fp16-fix VAE, the vendored loader
///    patches). The single-file `.safetensors` path is the INTENDED final
///    checkpoint (`image.safetensors`) but its sd.cpp load is known-broken
///    and is the NEXT fix target — so while both sit in the dir for testing,
///    the renderable layout must not be shadowed by the larger file (the
///    size-only selector did exactly that). Removing the GGUF after the
///    safetensors fix restores rule 2 → the checkpoint picks itself again.
/// 2. **Largest single file** otherwise (`.safetensors` or `.gguf`). SD
///    checkpoints don't have a locked naming convention (unlike the chat
///    model's `WUPI.gguf`), so size is the fallback selector.
///
...
/// one-strike latch — no correctness risk to the narrator path. Same for a
/// full-checkpoint GGUF misrouted to the multi-file path by a co-resident
/// CLIP-G: sd.cpp's loader errors cleanly and the latch surfaces it.
fn pick_sd_checkpoint(dir: &std::path::Path) -> Option<std::path::PathBuf> {
    let candidates: Vec<std::fs::DirEntry> = std::fs::read_dir(dir)
        ...
        .collect();
    let len_of = |e: &std::fs::DirEntry| e.metadata().ok().map(|m| m.len()).unwrap_or(0);
    // CLIP-G sibling sniff (std-only — this fn runs in default builds too,
    // where the diffusion-rs layout resolver isn't compiled in).
    let has_clip_g = candidates.iter().any(|e| {
        let p = e.path();
        p.extension()
            .and_then(|x| x.to_str())
            ...
            && p.file_stem()
                ...
                .map(|s| {
                    ...
                    s == "clip_g" || s == "clip-g" || s.starts_with("text_encoder_2")
                })
                .unwrap_or(false)
    });
    if has_clip_g {
...
async fn route_to_fable_manager(
    ...
    // Returns Ok(true) when a mutation was applied + the confirmation was
    // streamed; Ok(false) when the translation came back EMPTY (the caller
    // falls through to the normal Wupi chat body — a benign phrase that
    // merely LOOKED like a mutation must never eat the chat reply).
    ...
    //     the outcome exactly like the delta task's failure arm — consumed
    //     with a successful fold, budget-bumped alongside the new carrier on
    //     a retryable failure, restored unbumped on an infra failure. The old
    //     unconditional drop gave each prior ONE folded re-attempt before
    //     vanishing.
    if let Some(failed) = reply.failed_attempt.clone().and_then(bump_retry_budget) {
        for prior in deferred {
            if let Some(p) = bump_retry_budget(prior) {
                enqueue_failed_translation(state, p).await;
            }
        }
        enqueue_failed_translation(state, failed).await;
    } else if !reply.error.is_empty() {
        for prior in deferred {
            enqueue_failed_translation(state, prior).await;
        }
    } else {
        drop(deferred);
    }
    ...
    //    (P2 fix) An EMPTY translation is NOT a mutation — return false so
    //    chat_send falls through to the normal Wupi chat body. The old canned
    //    "couldn't turn that into a state change" replaced the chat reply on
    //    benign phrases ("turn around", "set the mood with music").
    let confirmation = if delta_applied {
        format_confirmation(&delta, &text)
    } else {
        tracing::info!(
            request = %text.chars().take(80).collect::<String>(),
            "fable-manager translation empty — falling back to normal chat"
        );
        return Ok(false);
    };
    on_event
    ...
    //    mutated state. Mirrors the `fable_send` autosave block.
    ...
    //    save → no disk churn).
    //    (2026-08-16 deferred-2) Snapshot-at-enqueue via the shared helper —
    //    the delta just applied above, so the captured pairing is exactly
        ...
        "fable_message_edit" => {
            ...
            if is_assistant {
                ...
                tokio::spawn(async move {
                    ...
                    if *state.active_card_id.lock().expect("active_card_id mutex") != expected_card {
                        tracing::warn!(
...
async fn fable_start(
    ...
    {
        let existing = state.active_fable_card.lock().expect("active_fable_card mutex");
        if existing.is_some() {
...
async fn enter_fable_session(
    ...
    // (P1 migration, 2026-08-15) Pre-normalization parses derived the id as
    // name.to_lowercase() - multi-word names produced space-separated state
    // folders while every write path used the dashed slug (the phantom-folder
    // split). Parse-time slug normalization makes the id canonical now; move
    // an existing legacy folder to the canonical name ONCE so its state,
    // codex, portrait, and saves all survive. Best-effort.
    {
        if let Some(root) = resolve_fable_cards_dir(app) {
            let legacy_id = card.name.to_lowercase();
            let legacy_safe = !legacy_id.is_empty()
                && !legacy_id.contains('/')
                && !legacy_id.contains('\\')
                && !legacy_id.contains("..")
                && !legacy_id.contains(':');
            if legacy_safe && legacy_id != card.id {
                let legacy_dir = root.join(&legacy_id);
                let canonical_dir = root.join(&card.id);
                if legacy_dir.is_dir() && !canonical_dir.exists() {
                    match std::fs::rename(&legacy_dir, &canonical_dir) {
                        Ok(()) => {
                            tracing::info!(from = %legacy_id, to = %card.id, "legacy card folder migrated to slug form");
                            // (2026-08-15 audit fix) The old folder's memory
                            // partition key (the pre-normalization id) is now
                            // an unreachable ghost — the runtime key is the
                            // normalized slug. Purge the dead rows so a same-
                            // name re-create can't inherit them; the card's
                            // codex re-seeds from the (renamed) .codex file.
                            if let Some(engine) = state.memory.get() {
                                match engine.purge_card_partition(&legacy_id).await {
                                    Ok(0) => {}
                                    Ok(n) => tracing::info!(
                                        card_id = %legacy_id,
                                        purged = n,
                                        "legacy partition purged after folder migration (rows were unreachable ghosts)"
                                    ),
                                    Err(e) => tracing::warn!(error = %format!("{e:#}"), "legacy partition purge after migration failed (non-fatal)"),
                                }
                            }
                        }
                        Err(e) => tracing::warn!(error = %e, "legacy card folder migration failed (continuing)"),
                    }
                }
            }
        }
...
fn fable_turn_in_flight(state: &AppState) -> bool {
    let narrator = state.active_fable_cancel.lock().expect("active_fable_cancel mutex");
    if narrator.is_some() {
        return true;
    }
    drop(narrator);
    let retrack = state.active_retrack_cancel.lock().expect("active_retrack_cancel mutex");
    retrack.is_some()
}
...
async fn fable_send(
    ...
    // lock nor the possible 3072-context engine spawn below.
    require_api_for_fable(&state)?;

    // (2026-08-15 audit fix) Server-side action cap: a giant paste would
    // front-drain the tracker's 3072-token context past its budget (the
    ...
    let (world_state, pacing, mut turn_directives) = {
        ...
        let recovery_directive: Option<String> = recovery_outcome.map(|r| {
            ...
            if let Some((part, new_state)) = &r.healed {
                if matches!(new_state, player_state::BodyPartState::Transparent) {
    ...
    if *state.active_card_id.lock().expect("active_card_id mutex") != card.id {
        tracing::warn!(
            turn_card = %card.id,
    ...
    // (2026-08-16 deferred-2) Routed through the snapshot-at-enqueue helper:
    // both halves are captured HERE — the turn's session install just
    // completed above, so the pairing is exactly this turn's committed
    // state, and the detached task takes no locks.
    spawn_reserved_autosave(&app, &state).await;

    // (2026-08-15 audit fix) The done event's send failure must NOT eat the
    // armed world tick: a dead channel means the frontend is gone, but the
    ...
    if let Err(e) = on_event.send(serde_json::json!({
        ...
        tracing::warn!(error = %e, "fable_send: done event send failed (dead channel); still firing the armed tick");
    }
...
/// and breaks cleanly (§2C KV-consistency contract). Signals BOTH fable-side
/// slots (2026-08-15 audit fix): the narrator turn's + the edit-retrack
/// pass's — the retrack runs detached + can overlap a Stage-2 stream, so a
/// stop during the overlap must halt whichever local decode is live.
#[tauri::command]
...
async fn fable_regenerate_slice(
    ...
    // (2026-08-15 audit fix) `target_content` is the identity fingerprint for
    // the post-wait re-validation below — the HTTP stream below is an
    // unguarded await during which a concurrent delete/edit/rewind/reroll can
    // shift the feed; splicing at the stale index would corrupt the WRONG
    // beat.
    let (prev_content, next_content, window, target_content): (Option<String>, Option<String>, Vec<session::Message>, String) = {
        let gs = state.fable_session.lock().await;
        let msgs = &gs.messages;
        let len = msgs.len();
        ...
        let target_content = _msg.content.clone();
    ...
    let Some(profile) = profile else {
        // Clear the slot (identity-checked, #46 — two overlapping slice
        // regens share this slot; the first finisher must never wipe the
        // second's live token), emit api_lost (mirror emit_fable_api_lost's
        // contract but for the slice slot — no autosave: a slice regen
        // mutates no world state, so there's nothing to preserve beyond
        // what's already saved).
        clear_cancel_slot_if_owner(&state.active_slice_cancel, &cancel);
        let _ = on_event.send(serde_json::json!({
            "type": "api_lost",
    ...
    // post-turn cleanup at the success exit). Identity-checked (#46): a
    // second pencil fire that overwrote the slot owns it.
    clear_cancel_slot_if_owner(&state.active_slice_cancel, &cancel);
    ...
    // Persist the splice — but ONLY onto the same beat we regenerated
    // (2026-08-15 audit fix). The HTTP wait above was unguarded: a concurrent
    // delete/edit/rewind/reroll during the regen can shift or replace the
    // message at `index`. The stale index would then splice the WRONG beat
    // (out-of-bounds errors cleanly via apply_slice_splice, but silently
    // corrupting a different beat is the real hazard). Re-validate identity
    // under the session lock: same index, same role, byte-identical content
    // (any mutation of this beat — edit, variant install, rewind — changes
    // it) + the same active card. On mismatch: soft error, splice nothing.
    let same_card = *state.active_card_id.lock().expect("active_card_id mutex") == card_id;
    {
        let mut gs = state.fable_session.lock().await;
        let still_target = gs
            .messages
            .get(index)
            .map(|m| m.role == session::Role::Assistant && m.content == target_content)
            .unwrap_or(false);
        if !(still_target && same_card) {
            tracing::warn!(
                index,
                "fable_regenerate_slice: the beat changed during regeneration (edit/rewind/exit); discarding the splice"
            );
            let _ = on_event.send(serde_json::json!({
                "type": "error",
                "message": "The beat changed while regenerating (it was edited, rewound, or the session ended). Re-select and try again.",
            }));
            return Ok(());
        }
...
async fn fable_end(
    ...
    // in flight — the same session-swap hazard fable_load_save guards
    // against: the turn's finalize would append its beat + restore its
    // schema onto the cleared/post-game state. The frontend already orders
    // stop-before-exit; this is the backend backstop.
    if fable_turn_in_flight(&state) {
        return Err("a narrator turn is still in flight: stop it (or wait) before ending the game".into());
    }
    // (2026-08-16 bug 8) Session boundary — invalidate in-flight ticks +
    // deferred retracks of the closing timeline (see the AppState field's
    // doc; the card-id check alone can't see a same-card restart).
    state
        .fable_session_generation
        .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
    ...
    // Belt-and-braces: only persist per-card state when a game is actually
    // active. A leaked active_card_id (e.g. a failed enter_fable_session)
    // must never pair with default/empty snapshots here — that combination
    // is exactly the save-wiper. Also makes a double fable_end a no-op
    // instead of a defaults-overwrite.
    let game_active = state
        .active_fable_card
        .lock()
        .expect("active_fable_card mutex")
        .is_some();
    if game_active && roleplay_card_id != memory::WUPI_CARD_ID {
        let schema_snapshot = state.fable_schema.lock().await.clone();
    ...
    *state
    ...
    // ("marcus returned from scouting — failure") + its retryable tick/
    // translation failures. Nothing cleared them at session boundaries, so
    // they survived into the NEXT game's first turn — the narrator obeyed
    // a foreign world's events as hard facts. Exiting already forfeits
    // them for this card (they live in app state, not the schema save);
    // the only question is which game they fire in, and the answer must
    // be "none".
    {
        let mut td = state.pending_tick_directives.lock().await;
        let dropped = td.len();
        td.clear();
        if dropped > 0 {
            tracing::warn!(dropped, "fable_end: dropped undelivered tick directives (session ended before their beat)");
        }
    }
...
async fn spawn_reserved_autosave(app: &tauri::AppHandle, state: &AppState) {
    ...
    tokio::spawn(async move {
        ...
        let result = tokio::task::spawn_blocking(move || {
            fable_save::write_save(&fable_root, &card, fable_save::AUTOSAVE_ID, "Autosave", &session, &schema)
        })
        .await;
        match result {
...
async fn edit_message(
    ...
    let is_assistant = {
        ...
        // The prose overwrite used to commit at ANY index and only the
        // (detached) retrack skipped non-trailing beats afterwards — a
        // mid-history assistant edit permanently diverged prose from world
        // state with no retrack and no autosave. User beats keep the pure
        // prose swap at any index (their regen path is rewind-and-edit).
        {
            let len = gs.messages.len();
            if let Some(m) = gs.messages.get(index) {
                if m.role == session::Role::Assistant && len > index + 1 {
...
async fn retrack_edited_assistant_message(
    ...
    *state.fable_schema.lock().await = revert_target;
    ...
    let cancel: llm::CancelToken = Arc::new(std::sync::atomic::AtomicBool::new(false));
    {
        let mut slot = state.active_retrack_cancel.lock().expect("active_retrack_cancel mutex");
        *slot = Some(Arc::clone(&cancel));
    }
...
async fn rewind_and_edit_user(
    ...
    // removed tail existed. (2026-08-15 audit fix: `>=`, not `>` — a
    // removed turn's REFEREE snapshot carries tag = its user message's
    // index, and a consecutive-user tail (interrupted regenerates leave
    // those) puts that index exactly AT surviving, so `>` kept it and the
    // removed turn's injuries survived the rewind. A surviving turn's
    // referee tag is always strictly < surviving — its user message sits
    // before the truncation point — so `>=` never drops a surviving-era
    // entry.) `schema_pop_count` returns 0 so the frontend's legacy pop
    // loop no-ops.
...
mod phase3_wiring_tests {
    ...
    fn wiring_tier_selection_defaults_to_soldier() {
        ...
        assert_eq!(
            player_state::select_attacker_tier_from_entities(&entities, &["marcus".to_string()]),
            player_state::AttackerTier::Soldier,
    ...
    fn wiring_tier_selection_picks_declared_legendary() {
        ...
        assert_eq!(
            player_state::select_attacker_tier_from_entities(&entities, &["dragon".to_string()]),
            player_state::AttackerTier::Legendary,
        );
        // (P2 scope fix) Off-camera tiers are IGNORED: the dragon not present
        // this turn must not escalate the fight.
        assert_eq!(
            player_state::select_attacker_tier_from_entities(&entities, &["goblin1".to_string()]),
            player_state::AttackerTier::Soldier,
            "off-camera legendary tier must be ignored"
        );
    }
    ...
    fn wiring_tier_selection_picks_highest_threat() {
        ...
        assert_eq!(
            player_state::select_attacker_tier_from_entities(
                &entities,
                &["thug1".to_string(), "dragon".to_string(), "goblin1".to_string()],
            ),
            player_state::AttackerTier::Legendary,
